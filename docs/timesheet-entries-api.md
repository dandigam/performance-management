...
No sowId or employeeId is needed: the weekly header identifies the setup and employee, and SOW is derived from the setup (null for internal work). `holidayId` is required for HOLIDAY and forbidden otherwise. For a LEAVE entry, `leaveId` can reference an approved leave request belonging to the timesheet employee. The request must include the work date, and entry hours must be positive and no greater than that date's approved requested hours. `leaveId` is forbidden for REGULAR and HOLIDAY entries. LEAVE entries with no `leaveId` remain supported.
