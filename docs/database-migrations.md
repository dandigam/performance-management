...
The Java startup migration runs after Hibernate has updated the schema. It converts legacy
assignment status `ACTIVE` to `ASSIGNED`, then derives each position's status as
`ASSIGNED`, `CLOSED`, or `OPEN`.
